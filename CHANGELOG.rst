...
- Add experimental NVFP4 routed-expert quantization support for MiniMax-M3-style inline-gated fused MoE experts. Generic HuggingFace fused experts no longer require an ``act_fn`` attribute when they expose the supported 3-D ``gate_up_proj`` / ``down_proj`` layout and ``num_experts``; this enables ``MiniMaxM3VLExperts`` to be wrapped by ``_QuantFusedExperts`` and calibrated/exported through the existing fused-MoE path with recipes such as ``general/ptq/nvfp4_experts_only-kv_fp8_cast``. MiniMax-M3 requires Transformers support outside ModelOpt's currently declared ``transformers>=4.56,<5.10`` dependency range.
